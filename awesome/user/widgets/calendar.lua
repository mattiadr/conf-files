...
function calendar:show(year, month)
	...
	if naughty.replace_text and self.notification and self.notification.box.visible and will_fit then
		naughty.replace_text(self.notification, title, text)
	else
		self:hide()
		local normal_no_size = {table.unpack(naughty.config.presets.normal)}
		normal_no_size.width = nil
		normal_no_size.height = nil
		self.notification = naughty.notify({
			preset = normal_no_size,
			title = title,
			text = text,
			timeout = 0,
